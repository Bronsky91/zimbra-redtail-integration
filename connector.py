...
def send_to_zimbra(act, created_in_redtail):
    ...
    e.add('dtend', redtail.parse_date(
        act['dtend'], act['allday'], user_timezone))
...
def sync():
    ...
    for act in redtail_cal:
        ...
        if To_Redtail.select().where(To_Redtail.redtail_act_id == act['uid']):
            if act['last_update'] > last_sync:
                ...
                redtail_act_id = db_record.get().redtail_act_id
                act['uid'] = db_record.get().zimbra_item_id
                send_to_zimbra(act, False)
                # Create new DB Record with Zimbra generated UID 
                To_Zimbra.create(
    ...
    for cal_item in zimbra_cal:
        # If Zimbra calendar item was sent to Redtail previously
        if To_Redtail.select().where(To_Redtail.zimbra_item_id == cal_item['uid']):
            if cal_item['last_update'] > last_sync:
                ...
                # Redtail act ID that was saved when sent
        ...
        # Else if Zimbra calendar item was sent to Zimbra from Redtail
        elif To_Zimbra.select().where(To_Zimbra.zimbra_item_id == cal_item['uid']):
            if cal_item['last_update'] > last_sync:
                db_record = To_Zimbra.select().where(
                    To_Zimbra.zimbra_item_id == cal_item['uid'])
                # Redtail ID that matches with the Zimbra cal that was sent to Redtail before
                redtail_recid = db_record.get().redtail_act_id 
                # Update Redtail activity
                redtail.put_cal_item(
                    rt_un, rt_pw, cal_item, redtail_recid, rt_user_id, timezone_offset)
    ...
    ### Temporarily disabling feature of Completing Redtail Acts using API ###
...
while True:
    ...
    timezone_offset=datetime.datetime.now(pytz.timezone(tz)).strftime('%z')
    print ' '
    print 'Sync Complete on {}'.format(
        pytz.utc.localize(datetime.datetime.now()).astimezone(user_timezone).strftime('%c')
        )
    print ' '
